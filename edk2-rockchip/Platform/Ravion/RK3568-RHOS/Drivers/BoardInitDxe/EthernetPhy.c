...
/* RHOS board use phy@1 on mdio interface */
#define PHY_ADDR	(1)
...
{
    PhyWrite (GmacBase, PHY_ADDR, PAGSR, 0xD04);
    MicroSecondDelay (10000);

    PhyWrite (GmacBase, PHY_ADDR, LCR, LCR_VALUE);
    MicroSecondDelay (10000);

    PhyWrite (GmacBase, PHY_ADDR, PAGSR, 0);

    /* ToDo: FixUp RHOS GMAC1 PHY clocking mode */
}
    ...
    )
{
    ...
    PhyRead (GmacBase, PHY_ADDR, MII_PHYIDR1, &PhyId[0]);
    PhyRead (GmacBase, PHY_ADDR, MII_PHYIDR2, &PhyId[1]);
    
    if (PhyId[0] == 0x2000 && PhyId[1] == 0xA240) {
        DEBUG ((DEBUG_INFO, "MDIO: Found IexasInstruments DP83822H GbE PHY\n"));
        DP83822HPhyInit (GmacBase);
    } else {
